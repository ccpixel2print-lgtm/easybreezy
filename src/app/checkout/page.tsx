...
import { checkPincode, submitCheckout } from '@/lib/api';
...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!form.contactName || !form.contactPhone || !form.addressLine1 || form.pincode.length !== 6 || !form.scheduledDate) {
    ...
    try {
      ...
      clear();
      setPlacedOrder({ orderNumber: order.orderNumber, totalAmount: order.totalAmount });
    } catch (err) {
        ...
        <main className="pt-[68px]">
          <section className="bg-cloud py-16">
            <div className="mx-auto max-w-md rounded-3xl bg-white p-10 text-center shadow-card ring-1 ring-black/5">
              ...
              <h1 className="mt-5 text-2xl font-extrabold text-brand">Order placed!</h1>
              <p className="mt-2 text-sm text-ink/70">
                Your order <strong className="text-ink">{placedOrder.orderNumber}</strong> has been created.
              </p>
              <p className="mt-1 text-sm text-ink/70">
                Amount: <strong className="text-ink">{inr(Math.round(placedOrder.totalAmount / 100))}</strong>
              </p>
              <p className="mt-4 rounded-lg bg-brand-tint px-3 py-2 text-xs text-brand">
                Payment integration is coming next — for now your order is saved as pending payment.
              </p>
              <Link
                href="/"
        ...
        <main className="pt-[68px]">
          <section className="bg-cloud py-16">
            <div className="mx-auto max-w-md rounded-3xl bg-white p-10 text-center shadow-card ring-1 ring-black/5">
              ...
              <Link href="/services" className="mt-6 inline-flex rounded-full bg-brand px-6 py-3 text-sm font-semibold text-white hover:bg-brand-dark">
                Browse Services
              </Link>
            </div>
          </section>
              ...
              <div className="space-y-6 lg:col-span-2">
                <div className="rounded-2xl bg-white p-6 shadow-card ring-1 ring-black/5">
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <input className={inputCls} placeholder="Phone number *" value={form.contactPhone} onChange={(e) => set('contactPhone', e.target.value.replace(/\D/g, '').slice(0, 10))} />
                    <input className={`${inputCls} sm:col-span-2`} placeholder="Email (optional)" value={form.contactEmail} onChange={(e) => set('contactEmail', e.target.value)} />
                  </div>
                </div>
                ...
                <div className="rounded-2xl bg-white p-6 shadow-card ring-1 ring-black/5">
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <input className={`${inputCls} sm:col-span-2`} placeholder="Address line 1 *" value={form.addressLine1} onChange={(e) => set('addressLine1', e.target.value)} />
                    <input className={`${inputCls} sm:col-span-2`} placeholder="Address line 2 (optional)" value={form.addressLine2} onChange={(e) => set('addressLine2', e.target.value)} />
                    <input className={inputCls} placeholder="Area / locality" value={form.area} onChange={(e) => set('area', e.target.value)} />
                    <input className={inputCls} placeholder="City *" value={form.city} onChange={(e) => set('city', e.target.value)} />
                    <input className={inputCls} placeholder="Pincode *" value={form.pincode} onChange={(e) => set('pincode', e.target.value.replace(/\D/g, '').slice(0, 6))} />
                  </div>
                </div>
                ...
                <div className="rounded-2xl bg-white p-6 shadow-card ring-1 ring-black/5">
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    />
                    <select className={inputCls} value={form.scheduledTimeWindow} onChange={(e) => set('scheduledTimeWindow', e.target.value)}>
                      {TIME_WINDOWS.map((w) => (
              ...
              <aside className="lg:col-span-1">
                <div className="sticky top-24 rounded-2xl bg-white p-6 shadow-card ring-1 ring-black/5">
                  ...
                  <ul className="mt-4 space-y-2 text-sm">
                    {items.map((i) => (
                      <li key={i.key} className="flex justify-between gap-2 text-ink/75">
                        <span>{i.name}{i.isHourly ? ` × ${i.quantity}h` : ''}</span>
                  ...
                  <dl className="mt-4 space-y-2 border-t border-dashed border-gray-200 pt-4 text-sm">
                    <div className="flex justify-between"><dt className="text-ink/70">Subtotal</dt><dd className="font-semibold">{inr(subtotal)}</dd></div>
                    <div className="flex justify-between"><dt className="text-ink/70">GST (18%)</dt><dd className="font-semibold">{inr(gst)}</dd></div>
                    <div className="flex justify-between border-t border-dashed border-gray-200 pt-2"><dt className="text-base font-bold">Total</dt><dd className="text-xl font-extrabold text-brand">{inr(total)}</dd></div>
                  </dl>
                  <button
                    type="submit"
                    disabled={loading}
                  ...
                  <p className="mt-3 text-xs text-ink/55">
                    Payment will be added next. For now your order is saved as pending payment.
                  </p>
                </div>
              </aside>
            </form>
